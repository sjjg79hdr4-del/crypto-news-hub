...
First, explain in clear Sinhala exactly WHAT THE NEWS IS SAYING (News Summary).
Then, analyze how this specifically impacts BITCOIN (BTC) spot/perp markets, liquidity, and orderbook.
...
Respond strictly with a valid JSON object matching this schema:
{
  ...
  "news_summary": "මෙම News එකෙන් හෝ Tweet එකෙන් සැබවින්ම කියවෙන්නේ කුමක්ද යන්න ඕනෑම කෙනෙකුට පැහැදිලි වන සරල හා නිවැරදි සිංහලෙන් විස්තර කරන්න.",
...
async def analyze_news(text):
    prompt_payload = f"Analyze the full content of this breaking news/tweet. First summarize what it says in Sinhala, then analyze BTC impact:\n\n{text[:3500]}"
    try:
        ...
        return {
            ...
            "bias_badge": "NEUTRAL",
            "news_summary": "මෙම පුවත සාමාන්‍ය වෙළඳපල හෝ දේශපාලන ප්‍රකාශනයක් වන අතර, ක්ෂණික ආර්ථික වෙනසක් වාර්තා වී නොමැත.",
...
async def news_worker():
    while True:
        ...
        payload = {
            ...
            "news_summary": res.get("news_summary", ""),
...
SYSTEM_PROMPT = """You are an institutional cryptocurrency quantitative macro strategist.
Carefully read the ENTIRE content of the breaking news or tweet.
1. Create a CONCISE, PUNCHY ENGLISH HEADLINE (summarized_english_title) summarizing the core event in English. Max 1-2 sharp sentences. No bloated text.
2. Analyze how this specifically impacts BITCOIN (BTC) spot/perp markets, liquidity, and orderbook.
Write ALL explanations, summaries, descriptions, Verdict, and Action Plan strictly in fluent, natural, institutional Sinhala.

Respond strictly with a valid JSON object matching this schema:
{
  "summarized_english_title": "Short, sharp, professional English headline summarizing the event (e.g. Band Protocol Reports Record $406M DeFi Exploits in Q2 2026)",
  "impact_mark": "උදා: 1.5 / 10 — NOISE හෝ 8.5 / 10 — CRITICAL ALPHA",
  "directional_bias": "NEUTRAL හෝ BULLISH හෝ BEARISH",
  "expected_move": "±$0-$50 හෝ ±$500-$1,200",
  "window": "ක්ෂණික තත්පර 60 තුළ හෝ විනාඩි 5-15 තුළ හෝ පැය 1-4 තුළ",
  "bias_badge": "NEUTRAL හෝ BULLISH හෝ BEARISH",
  "news_summary": "මෙම News එකෙන් හෝ Tweet එකෙන් සැබවින්ම කියවෙන්නේ කුමක්ද යන්න ඕනෑම කෙනෙකුට පැහැදිලි වන සරල හා නිවැරදි සිංහල විස්තරය.",
  "core_catalyst": "මෙම පුවතේ සඳහන් කරුණු BTC වලට සෘජු macro/structural catalyst එකක් වන්නේ ඇයි/නොවන්නේ ඇයිද යන්න සවිස්තරාත්මකව.",
  "cvd_orderbook_impact": "Spot CVD (Cumulative Volume Delta), Orderbook bid/ask walls, Aggressive market orders සහ DXY සම්බන්ධතාවයට වන බලපෑම.",
  "liquidity_traps": "Short/Long liquidations, Fake Wick අවදානම සහ Liquidity Hunt/Sweep එකක් සිදුවිය හැකි ආකාරය.",
  "verdict": "නොසලකා හරින්න (IGNORE) හෝ LONG BIAS හෝ SHORT BIAS හෝ තහවුරු වන තුරු රැඳී සිටින්න (WAIT)",
...
async def analyze_news(text):
    prompt_payload = f"Analyze the full content of this breaking news/tweet. Create a concise summarized English headline, and analyze BTC impact in Sinhala:\n\n{text[:3500]}"
    try:
        completion = await client.chat.completions.create(
            model=MODEL_NAME,
            messages=[
...
async def news_worker():
    while True:
        ...
        # Display the sharp summarized English title, or fallback to cleaned title
